...
const Events = () => {
  ...
  // Sort events by date
  const sortedEvents = [...upcomingEvents].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
        ...
        <div className="container mx-auto px-4">
          <div className="relative">
            ...
            <div className="space-y-12">
              {sortedEvents.map((event, index) => <motion.div key={event.id} className={`flex flex-col md:flex-row items-center gap-8 ${index % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse"}`} initial={{
              opacity: 0,
              x: index % 2 === 0 ? -50 : 50
            }} whileInView={{
              opacity: 1,
              ...
              delay: index * 0.1
                  ...
                  <div className={`w-full md:w-5/12 ${index % 2 === 0 ? "md:text-right" : "md:text-left"}`}>
                    <motion.div className="bg-card rounded-2xl p-6 shadow-card border border-border hover:shadow-elevated transition-all duration-300 group" whileHover={{
                  scale: 1.02,
                  y: -5
                }}>
                  ...
                  <div className="relative z-10 hidden md:flex items-center justify-center">
                    <motion.div className="w-4 h-4 rounded-full bg-primary shadow-lg shadow-primary/50" whileHover={{
                  scale: 1.5
                }} transition={{
                  duration: 0.2
                }} />
                  ...
                  <div className={`w-full md:w-5/12 ${index % 2 === 0 ? "md:text-left" : "md:text-right"}`}>
                    <motion.p className="text-primary font-display font-semibold text-lg" whileHover={{
                  scale: 1.05
                }}>
